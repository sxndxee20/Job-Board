...
function ApplyPage() {
  ...
  const { jobs, getJobById, user, submitApplication } = useApp();
  const fallbackJob = jobs[0];
  const job = getJobById(jobId) ?? fallbackJob;
    ...
    <div className="min-h-screen bg-background pb-28">
      <div className="mx-auto max-w-lg">
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border bg-background/90 px-5 py-3 backdrop-blur">
          <Link to="/jobs/$id" params={{ id: job?.id ?? "1" }} className="flex h-10 w-10 items-center justify-center rounded-xl border border-border bg-surface">
        ...
        <form onSubmit={handleSubmit} className="px-5 pt-6 space-y-7 pb-24">
          ...
          <Section title="Resume & Documents">
            <FileBox label="Resume / CV" fileName={resumeName} onFile={(f) => setResumeName(f.name)} required />
            <FileBox label="Cover Letter (Optional)" fileName={coverName} onFile={(f) => setCoverName(f.name)} />
          </Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section>
      <h2 className="mb-3 font-display text-sm font-bold uppercase tracking-wide text-muted-foreground">{title}</h2>
      <div className="space-y-3">{children}</div>
    </section>
  );
}

function FileBox({ label, fileName, onFile, required }: { label: string; fileName: string; onFile: (f: File) => void; required?: boolean }) {
  return (
    <div>
